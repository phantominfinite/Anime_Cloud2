...
# Mock Redis to avoid real connection attempts during tests
@pytest.fixture(autouse=True)
def mock_redis():
    with patch("app.services.downloader.redis") as mock_redis_lib:
        mock_client = MagicMock()
        mock_lock = AsyncMock()
        mock_lock.acquire.return_value = True
        mock_lock.locked = MagicMock(return_value=True)
        mock_lock.release = AsyncMock()
        mock_client.lock.return_value = mock_lock
        mock_redis_lib.from_url.return_value = mock_client
        yield mock_redis_lib
